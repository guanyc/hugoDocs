---
title: 前端设置 Front Matter
linktitle: 前端设置
description: Hugo允许在内容文件中添加yaml,TOML或者json格式的前端设置部分
date: 2017-01-09
...
前端设置** 允许您将元数据绑定到[内容类型][content type]的实例---嵌入到内容文件---这也是Hugo众多特性中的一个, 这些特性使得Hugo无比强大
...
Hugo支持四种前端设置格式,每种格式都有它们自己的标记符号.
...
: 一组Org mode 关键字, 格式是 '`#+KEY: VALUE`'. 任何不以 `#+` 开头的一行结束前端设置部分.
...
级联信息 cascade
前端设置的键值对的字典,它们的值会传递给页面的后代，除非被后代改写或者被后代的近端祖先的cascade改写.
详细信息参考[前端设置级联](#front-matter-cascade).

日期 date
: 页面的日期, 这通常由前端设置的`date`字段读取, 也是可配置的
...
layout
: hugo从布局[查询顺序][lookup]中选择用来呈现内容的布局。如果在前端设置中没有声明`layout`,hugo会在布局目录中查找和内容区块同名的布局文件。参考 ["定义内容类型"][definetype]
...
slug
: 呈现为输出的URL的尾部。在前端设置的slug值会覆盖从文件名称推断出来的URL的对应部分.
...
type
: 内容的类型。这个值如果在前端设置没声明，会自动从目录([区块][section])推断出来
...
\<taxonomies\>
: 复数形式的索引的域名称。参考上面前端设定例子中的 `tags` 和 `categories`.
_注意复数形式的用户定义的标签不能和hugo预定义的任何前端设置变量相同。
...
可以在前端设定中随意添加域以满足建立站点需求. 这些用户定义的键值对可以在站点模板中通过一个 `.Params` 变量访问使用.
...
任何节点和区块可以向下传递任何在只要保留在 `cascade` 前端设置键下面的前端设置值。
...
可以通过在页面的前端设置中对页面赋值内容特定的`weight`. 这些值对于列表的[排序ordering][ordering]特别有用。
...
可以在内容的前端设置中设置一些Markdown输出的选项，作为全局设定的设置覆盖。
